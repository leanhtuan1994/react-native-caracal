...
```
src/app/
├── _layout.tsx              # Root layout (providers, fonts, route guards)
├── onboarding.tsx           # First-launch onboarding (/onboarding)
├── (auth)/                  # Signed-out screens (stack)
│   ├── _layout.tsx
│   ├── login.tsx            # Sign in (/login)
│   └── sign-up.tsx          # Create account (/sign-up)
└── (home)/                  # Signed-in app (native tabs)
    ├── _layout.tsx          # NativeTabs: Home, Profile, Settings
    ├── index.tsx            # Home tab (/)
...
The root layout shows exactly one of three areas, chosen by `Stack.Protected` guards:

- **`onboarding`**: visible only on first launch (`useIsFirstTime()`).
- **`(auth)` group**: visible while signed out. Holds Sign in and Create account.
- **`(home)` group**: visible while signed in. Holds the three tabs.

The guard values come from the pure function `getRouteGuards({ isFirstTime, status })` in `src/lib/navigation/route-guards.ts`, which has unit tests.

<Callout type="info">
Route groups (folders with parentheses) organize your code without affecting URLs, so `(home)/index.tsx` is served at `/` and `(auth)/login.tsx` at `/login`.
</Callout>
...
The `(home)` group uses `NativeTabs` from `expo-router/unstable-native-tabs`, so iOS 26 renders the system liquid glass tab bar.

**Tabs:**
- **Home** (`index.tsx`): posts feed with search and tag filters
- **Profile** (`profile.tsx`): the signed-in account
- **Settings** (`settings.tsx`): language, theme, links and logout

Each `NativeTabs.Trigger` sets a translated `NativeTabs.Trigger.Label`, an SF Symbol / Material icon, and a translated `accessibilityLabel`. Pass the label explicitly: without it, VoiceOver keeps the tab names from the first language after the user switches language.

## Stack Navigation

The `(auth)` group uses a plain `Stack` (headers hidden) so Sign in and Create account can push and go back.
...
Screens are protected with `Stack.Protected` in `src/app/_layout.tsx`. Auth is hydrated synchronously from MMKV at module scope, so the first render already knows the status and no redirect flashes.
...
Signing in or out only changes the auth store; the guards then move the user to the right group, so screens never call `router.replace` after authentication.

...
To use your own palette, edit the token values in `src/themes/sky.css` (or add another theme file and import it from `global.css`). The tokens back Tailwind class names such as `bg-background`, `text-foreground`, and `bg-accent`, and HeroUI Native components read the same tokens, so both stay in sync. `global.css` also maps `--link` to a `text-link` utility, which the app uses for inline links.

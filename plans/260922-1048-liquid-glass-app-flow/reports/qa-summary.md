...
| `pnpm e2e-test` (Maestro 2.9.0, iOS 26.5)               | Pass. Rerun on 2026-09-23 after the design fixes: 4/4 flows in 3m 31s (onboarding, sign-up, login-with-validation, tabs).                                                                                                                                          |
...
The suite was rerun after the design corrections and passed 4/4. Port 8081 was held by another project's Metro, which is what made the earlier attempt load the wrong bundle, so this app ran on port 8082 and the flows took the Metro address from the new `METRO_URL` parameter (`utils/launch-app.yaml`). That other Metro process was left running.
